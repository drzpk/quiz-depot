...
class EESeeder extends Seeder {
    ...
    private const TEST_URL = 'http://egzamin-informatyk.pl/e14-egzamin-zawodowy-test-online';
    private const RESULT_URL = 'http://egzamin-informatyk.pl/e14-odpowiedzi';
    ...
    /** Kategoria egzaminów zawodowych */
    ...
    public function run(DatabaseManager $manager) {
        ...
        $category = $manager->getCategory('inf_exam');
        if ($category !== null) {
        ...
        $quiz->name = 'Egzamin E.14';
        if (!$manager->createQuiz($quiz)) {
    ...
    private function download() {
        ...
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, self::TEST_URL);
        ...
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, self::RESULT_URL);
        ...
        $this->parse($container->item(0));

        return true;
    }

    private function parse($container) {
        $index = 1;
        ...
        foreach ($container->childNodes as $node) {
            ...
            if ($node->getAttribute('class') == 'obrazek' && $node->hasChildNodes()) {
                ...
                if (strpos($text, (string) $index) === 0) {
                    ...
                    $current_question->quiz = $this->quiz;
